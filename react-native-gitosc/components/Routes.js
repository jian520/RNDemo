...
const NavigationBarRouteMapper = {
        ...
        if(route.id === constant.scene.web.key) {
           ...
           return (<TouchableOpacity underlayColor={Colors.lineGray}
                              ...
                              onPress={() => {
                                ...
                                let url = 'https://git.oschina.net/' + route.obj.path_with_namespace;
                                //let image = route.obj.owner.new_portrait;
                                Share.open({
                                    share_text: message,
                                    share_URL:url,
    ...
    Title: function(route, navigator, index, navState) {
        ...
        if(route.id === constant.scene.famous.key) {
            ...
            return (
                <View style={[styles.searchBar, {width: ScreenWidth - 45, marginLeft: 40}]}>
                    <FontAwesome
                        name={'search'}
                        size={20}
                        style={styles.searchIcon}
        ...
        return (
            <Text style={[styles.navBarText,
                      ...
                      {width: 250, height: 40, textAlign: 'center',textAlignVertical:"center", marginLeft:marginLeft}]}
                    numberOfLines={1}>
...
const routes = {
    navigator(initialRoute) {
        return (
            <Navigator
                key = {"navigator_key_" + initialRoute}
                ref = {"navigator_" + initialRoute}
                ...
                tabLabel = {this.getRouteCfg(initialRoute)}
                />
        );
    },

    getRouteCfg(id) {
        let title = NavigationBarRouteMapper._getTitle({id: id});
        let tab = {tabName: 'Unknown', iconName: 'ios-home'};
        switch (id) {
            case constant.scene.project.key:
                tab = {tabName: title, iconName: 'ios-book', selectedIconName: 'ios-book-outline'}; //ios-book-outline
                break;
            case constant.scene.famous.key:
                tab = {tabName: title, iconName: 'ios-eye', selectedIconName: 'ios-eye-outline'};//ios-eye-outline
                break;
            case constant.scene.personal.key:
                tab = {tabName: title, iconName: 'ios-person', selectedIconName: 'ios-person-outline'};//ios-person-outline
                break;
        }
